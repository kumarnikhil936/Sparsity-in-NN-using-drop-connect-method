...
# 10 layer ( 9 hidden layers ) FFNN to classify MNIST data
...
def dropConnect(w1, w2, w3, w4, w5, w6, w7, w8, w9, w10, p):
    w1s = w1.shape
    w2s = w2.shape
    w3s = w3.shape
    w4s = w4.shape
    ...
    w9s = w9.shape
    w10s = w10.shape
    n = np.prod(w1s) + np.prod(w2s) + np.prod(w3s) + np.prod(w4s) + np.prod(w5s) + np.prod(w6s) + np.prod(w7s) + np.prod(w8s) + np.prod(w9s) + np.prod(w10s)
    maskBits = np.random.choice(np.arange(n), int(np.floor((1-p)*n)), replace=False)
    wf = np.concatenate((w1.flatten(), w2.flatten(), w3.flatten(), w4.flatten(), w5.flatten(), w6.flatten(), w7.flatten(), w8.flatten(), w9.flatten(), w10.flatten()), axis=0)
    wf[maskBits] = 0
    ...
    w8m = wf[(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)+np.prod(w6s)+np.prod(w7s)):(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)+np.prod(w6s)+np.prod(w7s)+np.prod(w8s))].reshape(w8s)
    w9m = wf[(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)+np.prod(w6s)+np.prod(w7s)+np.prod(w8s)):(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)+np.prod(w6s)+np.prod(w7s)+np.prod(w8s)+np.prod(w9s))].reshape(w9s)
    w10m = wf[(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)+np.prod(w6s)+np.prod(w7s)+np.prod(w8s)+np.prod(w9s)):].reshape(w10s)
    return w1m, w2m, w3m, w4m, w5m, w6m, w7m, w8m, w9m, w10m
...
for keep_percent in np.arange(1, 0.0, -0.05):  # dropping keep_percent in steps of 0.05 in every iteration
    ...
    w9 = np.genfromtxt('trained_w9.csv', delimiter="\t")
    b9 = np.genfromtxt('trained_b9.csv', delimiter="\t")
    w10 = np.genfromtxt('trained_w10.csv', delimiter="\t")
    b10 = np.genfromtxt('trained_b10.csv', delimiter="\t")

    w1, w2, w3, w4, w5, w6, w7, w8, w9, w10 = dropConnect(w1=w1, w2=w2, w3=w3, w4=w4, w5=w5, w6=w6, w7=w7, w8=w8, w9=w9, w10=w10, p=keep_percent)
    ...
    w9 = tf.Variable(w9, dtype=tf.float32, name='w9')
    w10 = tf.Variable(w10, dtype=tf.float32, name='w10')
