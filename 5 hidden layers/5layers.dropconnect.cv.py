...
# 6 layer ( 5 hidden layers ) FFNN to classify MNIST data
...
def dropConnect(w1, w2, w3, w4, w5, w6, p):
    w1s = w1.shape
    w2s = w2.shape
    w3s = w3.shape
    w4s = w4.shape
    w5s = w5.shape
    w6s = w6.shape
    n = np.prod(w1s) + np.prod(w2s) + np.prod(w3s) + np.prod(w4s) + np.prod(w5s) + np.prod(w6s)
    maskBits = np.random.choice(np.arange(n), int(np.floor((1-p)*n)), replace=False)
    wf = np.concatenate((w1.flatten(), w2.flatten(), w3.flatten(), w4.flatten(), w5.flatten(), w6.flatten()), axis=0)
    wf[maskBits] = 0
    ...
    w6m = wf[(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)):].reshape(w6s)
    return w1m, w2m, w3m, w4m, w5m, w6m
...
for keep_percent in np.arange(1, 0.0, -0.05):  # dropping keep_percent in steps of 0.05 in every iteration
    ...
    w1, w2, w3, w4, w5, w6 = dropConnect(w1=w1, w2=w2, w3=w3, w4=w4, w5=w5, w6=w6, p=keep_percent)
