...
# 7 layer ( 6 hidden layers ) FFNN to classify MNIST data
...
def dropConnect(w1, w2, w3, w4, w5, w6, w7, p):
    ...
    w6s = w6.shape
    w7s = w7.shape
    n = np.prod(w1s) + np.prod(w2s) + np.prod(w3s) + np.prod(w4s) + np.prod(w5s) + np.prod(w6s) + np.prod(w7s)
    maskBits = np.random.choice(np.arange(n), int(np.floor((1-p)*n)), replace=False)
    wf = np.concatenate((w1.flatten(), w2.flatten(), w3.flatten(), w4.flatten(), w5.flatten(), w6.flatten(), w7.flatten()), axis=0)
    wf[maskBits] = 0
    ...
    w7m = wf[(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)+np.prod(w6s)):].reshape(w7s)
    return w1m, w2m, w3m, w4m, w5m, w6m, w7m
...
for keep_percent in np.arange(1, 0.0, -0.05):  # dropping keep_percent in steps of 0.05 in every iteration
    ...
    w1, w2, w3, w4, w5, w6, w7 = dropConnect(w1=w1, w2=w2, w3=w3, w4=w4, w5=w5, w6=w6, w7=w7, p=keep_percent)
