...
# 9 layer ( 8 hidden layers ) FFNN to classify MNIST data
...
def dropConnect(w1, w2, w3, w4, w5, w6, w7, w8, w9, p):
    w1s = w1.shape
    w2s = w2.shape
    w3s = w3.shape
    w4s = w4.shape
    ...
    w8s = w8.shape
    w9s = w9.shape
    n = np.prod(w1s) + np.prod(w2s) + np.prod(w3s) + np.prod(w4s) + np.prod(w5s) + np.prod(w6s) + np.prod(w7s) + np.prod(w8s) + np.prod(w9s)
    maskBits = np.random.choice(np.arange(n), int(np.floor((1-p)*n)), replace=False)
    wf = np.concatenate((w1.flatten(), w2.flatten(), w3.flatten(), w4.flatten(), w5.flatten(), w6.flatten(), w7.flatten(), w8.flatten(), w9.flatten()), axis=0)
    wf[maskBits] = 0
    ...
    w9m = wf[(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)+np.prod(w5s)+np.prod(w6s)+np.prod(w7s)+np.prod(w8s)):].reshape(w9s)
    return w1m, w2m, w3m, w4m, w5m, w6m, w7m, w8m, w9m
...
for keep_percent in np.arange(1, 0.0, -0.05):  # dropping keep_percent in steps of 0.05 in every iteration
    ...
    w1, w2, w3, w4, w5, w6, w7, w8, w9 = dropConnect(w1=w1, w2=w2, w3=w3, w4=w4, w5=w5, w6=w6, w7=w7, w8=w8, w9=w9, p=keep_percent)
