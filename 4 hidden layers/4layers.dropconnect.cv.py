...
# 5 layer ( 4 hidden layers ) FFNN to classify MNIST data
...
# Arguments: w1, w2, w3, w4, w5 - Weight vectors
#            p - Percentage value (0.05 - 1) stating how much percentage of weights to preserve (1 - drop percentage)
def dropConnect(w1, w2, w3, w4, w5, p):
    w1s = w1.shape
    w2s = w2.shape
    w3s = w3.shape
    w4s = w4.shape
    w5s = w5.shape
    n = np.prod(w1s) + np.prod(w2s) + np.prod(w3s) + np.prod(w4s) + np.prod(w5s)
    maskBits = np.random.choice(np.arange(n), int(np.floor((1-p)*n)), replace=False)
    wf = np.concatenate((w1.flatten(), w2.flatten(), w3.flatten(), w4.flatten(), w5.flatten()), axis=0)
    wf[maskBits] = 0
    ...
    w5m = wf[(np.prod(w1s)+np.prod(w2s)+np.prod(w3s)+np.prod(w4s)):].reshape(w5s)
    return w1m, w2m, w3m, w4m, w5m
...
for keep_percent in np.arange(1, 0.0, -0.05):  # dropping keep_percent in steps of 0.05 in every iteration
    ...
    w1, w2, w3, w4, w5 = dropConnect(w1=w1, w2=w2, w3=w3, w4=w4, w5=w5, p=keep_percent)
